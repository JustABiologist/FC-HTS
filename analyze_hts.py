...
def parse_arguments():
    ...
    parser.add_argument("--peacoqc", action="store_true",
...
def run_peacoqc(fcs_files, output_dir):
    ...
    Returns path to directory containing QC'd files, or None if failed.
    
    PeacoQC removes:
    - Flow rate anomalies (clogs, speed changes)
    - Unstable time regions
    - Outlier events
    
    Reference: https://github.com/saeyslab/PeacoQC
    """
    print("\n" + "="*70)
    print("Running PeacoQC preprocessing...")
    print("="*70)
    
    # Create output directory for QC'd files
    qc_output_dir = os.path.join(output_dir, "peacoqc_cleaned")
    os.makedirs(qc_output_dir, exist_ok=True)
    
    # Create R script for PeacoQC processing
    r_script = '''
# PeacoQC preprocessing script
# Auto-generated by FC-HTS pipeline

# Check if PeacoQC is installed
if (!requireNamespace("PeacoQC", quietly = TRUE)) {
    message("ERROR: PeacoQC package not installed.")
    message("Install with: BiocManager::install('ComplexHeatmap'); devtools::install_github('saeyslab/PeacoQC')")
    quit(status = 1)
}
...
message(paste("Found", length(fcs_files), "FCS files to process"))

# Process each file
for (fcs_file in fcs_files) {
    tryCatch({
        message(paste("Processing:", basename(fcs_file)))
        
        # Read FCS file
        ff <- read.FCS(fcs_file, transformation = FALSE)
        
        # Run PeacoQC - remove anomalies based on time channel
        # Use default channels for QC (Time, FSC, SSC)
        channels_to_check <- colnames(ff)[grep("Time|FSC|SSC", colnames(ff), ignore.case = TRUE)]
        
        if (length(channels_to_check) > 0) {
            qc_result <- PeacoQC(
                ff,
            ...
            # Get the cleaned flowFrame
            ff_clean <- qc_result$FinalFF
            
            # Calculate how many events were removed
            original_events <- nrow(ff)
            cleaned_events <- nrow(ff_clean)
            removed_pct <- round((1 - cleaned_events/original_events) * 100, 1)
            message(paste("  Removed", removed_pct, "% of events (", original_events - cleaned_events, "events )"))
        } else {
            message("  No Time/FSC/SSC channels found, skipping QC")
            ff_clean <- ff
        }
        
        # Save cleaned FCS file
        output_file <- file.path(output_dir, basename(fcs_file))
        write.FCS(ff_clean, output_file)
        
    }, error = function(e) {
        message(paste("  Error processing file:", e$message))
        # Copy original file if QC fails
        file.copy(fcs_file, file.path(output_dir, basename(fcs_file)))
    })
}

message("PeacoQC preprocessing complete!")
'''
    
    # Write R script to temp file
    r_script_path = os.path.join(output_dir, "peacoqc_script.R")
    with open(r_script_path, 'w') as f:
        f.write(r_script)
    
    # Get input directory (assume all files are in same directory)
    input_dir = os.path.dirname(fcs_files[0]) if fcs_files else "."
    ...
    try:
        ...
        if result.returncode != 0:
            print(f"⚠️  PeacoQC failed with return code {result.returncode}")
            print("Continuing with original (unprocessed) FCS files...")
            # Clean up
            if os.path.exists(r_script_path):
                os.remove(r_script_path)
        ...
        # Check if QC'd files were created
        qc_files = glob.glob(os.path.join(qc_output_dir, "*.fcs"))
        if not qc_files:
            print("⚠️  No QC'd files created. Continuing with original files...")
            if os.path.exists(r_script_path):
                os.remove(r_script_path)
            return None
        
        print(f"✓ PeacoQC complete. {len(qc_files)} cleaned files in {qc_output_dir}")
        ...
        print("⚠️  Rscript not found. Is R installed and in PATH?")
        print("Continuing with original (unprocessed) FCS files...")
        if os.path.exists(r_script_path):
            os.remove(r_script_path)
        return None
    except subprocess.TimeoutExpired:
        print("⚠️  PeacoQC timed out after 10 minutes.")
        print("Continuing with original (unprocessed) FCS files...")
        if os.path.exists(r_script_path):
            os.remove(r_script_path)
        return None
    except Exception as e:
        print(f"⚠️  Error running PeacoQC: {e}")
        print("Continuing with original (unprocessed) FCS files...")
        if os.path.exists(r_script_path):
            os.remove(r_script_path)
        return None
...
def main():
    ...
    # 2b. Optional: Run PeacoQC preprocessing
    qc_dir = None
    if args.peacoqc and fcs_files:
        qc_dir = run_peacoqc(fcs_files, args.output)
        if qc_dir:
            # Use QC'd files instead of originals
            fcs_files = glob.glob(os.path.join(qc_dir, "*.fcs"))
            print(f"Using {len(fcs_files)} PeacoQC-cleaned files for analysis.")
    ...
    if args.intensity_floor is not None:
        ...
        legend_labels.append('Intensity Floor')
    ...
    # Clean up PeacoQC files if not keeping them
    if qc_dir and not args.keep_qc_files:
        try:
            shutil.rmtree(qc_dir)
            print(f"Cleaned up temporary QC files.")
        except Exception as e:
            print(f"Warning: Could not remove QC directory: {e}")
    elif qc_dir and args.keep_qc_files:
        print(f"QC'd FCS files saved in: {qc_dir}")
